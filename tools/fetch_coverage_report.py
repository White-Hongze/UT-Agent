...
def _render_report(parsed: dict, max_files: int = 20, max_lines_per_file: int = 80) -> str:
    ...
    if summary:
        parts = []
        if "total" in summary: parts.append(f"总修改行 {summary['total']}")
        if "covered" in summary: parts.append(f"已覆盖 {summary['covered']}")
        if "uncovered" in summary: parts.append(f"未覆盖 {summary['uncovered']}")
        if "coverage_pct" in summary: parts.append(f"覆盖率 {summary['coverage_pct']}%")
        if parts:
...
def _try_parse_summary(html: str) -> dict:
    """从 HTML 顶部抓 总修改/已覆盖/未覆盖/覆盖率 数字。
    注意：先剥掉 style="..." 属性，避免 #f44336 / #4CAF50 这类 hex 颜色被误吞为数字。
    """
    cleaned = re.sub(r'\sstyle\s*=\s*"[^"]*"', " ", html, flags=re.IGNORECASE)
    cleaned = re.sub(r"\sstyle\s*=\s*'[^']*'", " ", cleaned, flags=re.IGNORECASE)
    res: dict = {}
    m = re.search(r"总修改行数[\s\S]{0,200}?>\s*(\d+)\s*<", cleaned)
    if m: res["total"] = int(m.group(1))
    m = re.search(r"已覆盖行数[\s\S]{0,200}?>\s*(\d+)\s*<", cleaned)
    if m: res["covered"] = int(m.group(1))
    m = re.search(r"未覆盖行数[\s\S]{0,200}?>\s*(\d+)\s*<", cleaned)
    if m: res["uncovered"] = int(m.group(1))
    m = re.search(r"覆盖率[\s\S]{0,200}?>\s*(\d+(?:\.\d+)?)\s*%", cleaned)
    if m: res["coverage_pct"] = float(m.group(1))
    return res
...
def fetch_changed_lines_report(ut_job_id: int) -> dict:
    ...
    if not git_provider:
        return {"available": False, "reason": "git_provider 未初始化"}
    ...
    try:
        ...
        return {"available": False, "reason": f"获取项目失败: {e}"}

    try:
        job = project.jobs.get(ut_job_id)
    except Exception as e:
        return {"available": False, "reason": f"获取 job #{ut_job_id} 失败: {e}"}

    try:
        raw = job.artifact(ARTIFACT_PATH)
    except Exception as e:
        return {"available": False, "reason": f"拉取 artifact {ARTIFACT_PATH} 失败: {e}"}
