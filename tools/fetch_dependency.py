...
def _write_dependency_file(content: str, file_path_in_repo: str, output_dir: str, mr_id: int) -> str:
    """将单个依赖文件内容落盘到 workspace/mr_{id}/deps/ 目录。"""
    deps_dir = os.path.join(output_dir, f"mr_{mr_id}", "deps")
...
def fetch_dependency(git_provider, file_path: str, output_dir: str, mr_id: int, branch: Optional[str] = None) -> str:
    ...
    try:
        ...
        # 直接落盘
        local_path = _write_dependency_file(content, file_path, output_dir, mr_id)
        return local_path
